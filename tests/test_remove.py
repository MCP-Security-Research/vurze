"""Tests for the 'pyseal remove' CLI command."""

import os
import tempfile
import textwrap
from pyseal.remove_decorators import remove_decorators

SAMPLE_DECORATED_CODE = textwrap.dedent('''
@pyseal._abcdef123456()
def foo():
	return 42
...
def test_remove_decorators_removes_all():
	"""Test that remove_decorators removes all pyseal decorators from functions and classes."""
	with tempfile.TemporaryDirectory() as tmpdir:
		...
		assert found, "Should find pyseal decorators to remove"
		assert "@pyseal" not in modified_code, "Decorators were not fully removed"
...
def test_remove_decorators_handles_no_decorators():
	"""Test that remove_decorators does nothing if no pyseal decorators are present."""
	with tempfile.TemporaryDirectory() as tmpdir:
		file_path = os.path.join(tmpdir, "plain.py")
		with open(file_path, "w") as f:
			f.write(SAMPLE_UNDECORATED_CODE)
		modified_code, found = remove_decorators(file_path)
		assert not found, "Should not find any pyseal decorators"
...
def test_remove_decorators_mixed_decorators():
	"""Test that remove_decorators only removes pyseal decorators and leaves others."""
...
''')
	with tempfile.TemporaryDirectory() as tmpdir:
		...
		with open(file_path, "w") as f:
			f.write(mixed_code)
		modified_code, found = remove_decorators(file_path)
		assert found, "Should find pyseal decorator to remove"
		assert "@pyseal" not in modified_code, "Pyseal decorator was not removed"
